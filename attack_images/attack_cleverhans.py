...
def call_function_by_attack_name(attack_name):
    if attack_name not in ["CW", "PGD", "BIM", "FGSM"] :
...
def gen_adv_data(model, x, y, attack_name, dataset_name, batch_size=2048):
    ...
    attack = call_function_by_attack_name(attack_name)(model_wrap, sess=sess)
        ...
        if dataset_name in model_dict:
            for model_name in model_dict[dataset_name]:
                for attack_name in attack_names :
                    ...
                    print(
                        "Model accuracy on benign images: {:.2f}%".format(acc))
                    ...
                    print(
                        "Model accuracy on adversarial images: {:.2f}%".format(acc))
